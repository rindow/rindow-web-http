...
class Uri implements UriInterface
{
    ...
    public function __construct($uri=null, $secret=null)
    {
        if($secret!=null)
            $this->secret = $secret;
        else
    ...
    {
        if(!is_string($uri))
            throw new Exception\InvalidArgumentException('Type of uri must be string.: '.(is_object($uri)? get_class($uri) : gettype($uri)));
        $parts = parse_url($uri);
    ...
    public function withScheme($scheme)
    {
        ...
        if($scheme!='http' && $scheme!='https')
            throw new Exception\InvalidArgumentException('Invalid scheme.: '.$scheme);
        $message = clone $this;
        $message->set('scheme',$scheme,$this->secret);
        return $message;
    }
    ...
    public function withUserInfo($user, $password = null)
    {
        ...
        $message = clone $this;
        $message->set('user',$user,$this->secret);
        $message->set('password',$password,$this->secret);
        return $message;
    }
    ...
    public function withHost($host)
    {
        if(!empty($host))
            ...
            $host = '';
        $message = clone $this;
        $message->set('host',$host,$this->secret);
        return $message;
    }
    ...
    public function withPort($port)
    {
        if(!empty($port)) {
            if(!is_numeric($port))
                throw new Exception\InvalidArgumentException('Invalid port.: '.$port);
        } else {
            $port = null;
        }
        $message = clone $this;
        $message->set('port',$port,$this->secret);
        return $message;
    }
    ...
    public function withPath($path)
    {
        ...
        $message = clone $this;
        $message->set('path',$path,$this->secret);
        return $message;
    }
    ...
    public function withQuery($query)
    {
        if(!empty($query))
            ...
            $query = '';
        $message = clone $this;
        $message->set('query',$query,$this->secret);
        return $message;
    }
    ...
    public function withFragment($fragment)
    {
        if(!empty($fragment))
            ...
            $fragment = '';
        $message = clone $this;
        $message->set('fragment',$fragment,$this->secret);
        return $message;
    }
